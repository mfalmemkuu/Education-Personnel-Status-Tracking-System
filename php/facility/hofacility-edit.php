...
$statement = $conn->prepare("SELECT hof.FacilityID, m.PresidentMedicareNumber, f.Name,  
                                    f.WebAddress, f.Capacity, f.PostalCode, f.PhoneNumber, wa.StartDate
                             FROM Facilities f, ManagementFacilities m, Persons p , HeadOfficeFacilities hof, Works_at wa
                             WHERE m.FacilityID = f.FacilityID AND m.PresidentMedicareNumber = p.MedicareCardNumber 
                             AND hof.FacilityID = f.FacilityID AND wa.FacilityID=f.FacilityID AND m.FacilityID=:FacilityID;");
        ...
        if ($facility->execute()) {
            ...
            if ($managementfacility->execute()) {
                ...
                if($works_at->execute()) {
                    ...
                    echo "Error editing works at record.";
                }
            } else {
...
<body>
    ...
    <form action="./hofacility-edit.php" method="post">
