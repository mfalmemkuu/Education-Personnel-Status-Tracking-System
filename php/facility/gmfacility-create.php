...
$facility->bindParam(':PhoneNumber',$_POST["PhoneNumber"]);
    if ($facility->execute()) {
        ...
        if($works_at->execute()) {
          $generalmanagement = $conn->prepare("INSERT INTO GeneralManagementFacilities(FacilityID)
                                        VALUES(:FacilityID);");
          $generalmanagement->bindParam(':FacilityID', $_POST["FacilityID"]);
          if ($generalmanagement->execute()) {
            header("Location: ../facility/index.php"); 
            exit; 
          }
          else{
            echo "Error adding general management facility";
          }
        }
        else{
...
<body>
    ...
    <h1>Add a General Management Office</h1>
    <form action="gmfacility-create.php" method="post">
