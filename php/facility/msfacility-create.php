    ...
    $facility->bindParam(':PhoneNumber',$_POST["PhoneNumber"]);
        ...
        if ($facility->execute()) {
            ...
            $educationalfacility->bindParam(':PrincipalMedicareNumber', $_POST["PrincipalMedicareNumber"]);
                ...
                if($works_at->execute()){
                $middleschool = $conn->prepare("INSERT INTO MiddleSchools(FacilityID) VALUES(:FacilityID);");
                $middleschool->bindParam(':FacilityID', $_POST["FacilityID"]);
                if($middleschool->execute()){
                        header("Location: ../facility/index.php"); 
                        exit; 
                }
                else{
                    echo "error creating middleschool";
                }
                }
                else{
                    echo "error creating works at";
                }
            } 
            else {
                echo "Error creating educational facility.";
            }
        } 
        else {
...
<body>
    ...
    <h1>Add a Middle School</h1>
    <form action="msfacility-create.php" method="post">
